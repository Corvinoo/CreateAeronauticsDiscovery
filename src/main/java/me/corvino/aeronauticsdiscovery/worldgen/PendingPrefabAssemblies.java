...
public class PendingPrefabAssemblies extends SavedData {
    ...
    private void process(ServerLevel level) {
        ...
        while (listIterator.hasNext() && processed < MAX_ASSEMBLIES_PER_TICK) {
            ...
            Optional<String> failing = PrefabReadinessChecks.firstFailing(level, entry.bounds);
            if (failing.isPresent()) {
                CreateAeronauticsDiscovery.LOGGER.info(
                        "[PROCESS] Entry at {}: readiness check '{}' not yet passing (attempt {}/{})",
                        entry.assemblerPos(), failing.get(), entry.retryCount + 1, MAX_RETRIES
                );
